...
### Feature group isolation (Feb 2026, 229 events)
...
- **All groups: R² < 0** — no group outperforms predicting the training mean
...
- LSTM (sequence model, pregame tier) achieves R²=+0.033±0.035 — only model with positive R², consistent across folds 1 and 2; tabular models all R²<0
...
- Conclusion: data volume is the bottleneck, not feature engineering — ~200 events is insufficient to surface weak signal reliably; LSTM architecture worth prioritising when data grows
