...
| **ESPN API (for lineups)** | ESPN summary endpoint returns roster data for completed matches but was empty ~10h pre-KO in testing. Also returns 403 from cloud/datacenter IPs (remote triggers). Works from residential IP via local Python/Playwright. Timing at KO-60 unconfirmed — EPL test inconclusive. Existing infra (EspnLineup model, reader/writer) could be used if timing is proven. |
